...
17th Jan 2004 0.5  Added timeout_ms param to watcher object call; this allows
                    for the possibility of pumping for waiting messages to
                    preventing, eg, the PythonWin IDE locking up. See the
                    docstring for the watch_for method.
...
#
# Do just enough to ensure constants are available
#
...
WMI_EXCEPTIONS = {
  win32com.client.constants.wbemErrInvalidQuery : x_wmi_invalid_query,
  win32com.client.constants.wbemErrTimedout : x_wmi_timed_out
}
  ...
  def __init__ (self, ole_object, method_name):
    try:
      ...
      if self.in_parameters is None:
        ...
        self.in_parameter_names = [i.Name for i in self.in_parameters.Properties_]
      if self.out_parameters is None:
        self.out_parameter_names = []
      else:
        self.out_parameter_names = [i.Name for i in self.out_parameters.Properties_]

      self.__doc__ = "%s (%s) => (%s)" % (method_name, ", ".join (self.in_parameter_names), ", ".join (self.out_parameter_names))
  ...
  def __call__ (self, **kwargs):
    ...
    try:
      if self.in_parameters:
        ...
        parameter_names = self.in_parameter_names
      ...
      return tuple ([result.Properties_ (name) for name in self.out_parameter_names])
...
class _wmi_watcher:
  ...
  def __call__ (self, timeout_ms=-1):
    ...
    try:
      ...
      if additional_info:
        wcode, source_of_error, error_description, whlp_file, whlp_context, scode = additional_info
        if scode == win32com.client.constants.wbemErrTimedout:
...
class WMI:
  ...
  def __init__ (
    ...
    try:
      ...
      _set (self, "wmi", win32com.client.gencache.EnsureDispatch (wmi._oleobj_))
  ...
  def query (self, wql):
    ...
    flags = constants.wbemFlagReturnImmediately | constants.wbemFlagForwardOnly
    wql = wql.replace ("\\", "\\\\")
    try:
      return [
...
def Registry (
  ...
  try:
    ...
    return _wmi_object (win32com.client.gencache.EnsureDispatch (registry))
