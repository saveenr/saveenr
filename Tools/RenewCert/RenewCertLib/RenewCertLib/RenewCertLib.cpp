...
class X
{
	...
	void MakeNewCert(HCRYPTPROV hCPContext, LPCWSTR szCertName, LPCWSTR szPassword, CRYPT_DATA_BLOB *pPFX, CRYPT_KEY_PROV_INFO Info)
	{
		...
		int years_to_extend = 100;

		GetSystemTime(&certExpireDate);
		certExpireDate.wYear += years_to_extend;
	...
	void RenewCert(int argc, LPCWSTR szCertFileName, LPCWSTR szCertName, LPCWSTR szPassword, LPCWSTR szOutFile  )
	{
